...
`;var Ye=class extends D{static get properties(){return{hass:{attribute:!1},_config:{state:!0},_translations:{state:!0}}}static get styles(){return oe`
            ...
            <div class="modal-body" tabindex="0">
              ${o.length>0?h`
                    <div class="calendar-list">
                      ${o.map((c,i)=>{let m=c.home_team||c.equipe_domicile||"-",g=c.away_team||c.equipe_exterieur||"-",A=c.home_logo||w,L=c.away_logo||w,k=c.score||(c.home_score!==void 0?`${c.home_score} - ${c.away_score}`:""),N=this._formatDate(c.date||c.datetime),R=l(m),S=l(g),b=R||S,y=!!(c.is_played||k),x=i===u,f="";return R?f="DOM":S&&(f="EXT"),h`
                          <div
                            class="calendar-row ${b?"highlight-row":""} ${x?"next-match-row":""}"
                            @click=${()=>this._selectCalendarMatch(i,y)}
                            @keydown=${this._onKeyActivate(()=>this._selectCalendarMatch(i,y))}
                            role="button"
                            tabindex="0"
                            aria-label="${m} vs${g}"
                          >
                            ...
                            <div class="calendar-col-meta">
                              ${x?h`<div class="cal-badge-next">Prochain</div>`:""}
    ...
    `}_renderMatchHeader(e){let{competition:t,pouleName:a,roundNumber:o}=e;return h`
      ${this._config.show_header?h`
          ...
          `:""}
    `}_renderMatchArea(e){let{isLive:t,canToggleView:a,isPostMatch:o,isGameDay:n,leftName:s,rightName:d,leftLogo:l,rightLogo:u,leftUrl:c,rightUrl:i,leftEntityId:m,rightEntityId:g,gymName:A,gymCity:L,dateFormatted:k,logoSizeClass:N,showRank:R,leftRank:S,rightRank:b,isCalendarClickable:y,isLogoClickable:x,hasStandingsData:f}=e,C=this._config?.rank_badge_style==="solid"||!!this._config?.solid_rank_badges?"rank-solid":"";return h`
      <div class="match-area">
        <div class="team-logo-cell cell-left">
          <div
            class="logo-box ${N} ${x?"clickable":""}"
            @click=${()=>this._handleLogoClick(m,c,s)}
            @keydown=${x?this._onKeyActivate(()=>this._handleLogoClick(m,c,s)):p}
            role=${x?"button":p}
            tabindex=${x?"0":p}
        ...
        <div class="center-meta-wrapper">
          ...
          <div
            ...
            @click=${()=>{y&&this._openCalendar(e.targetDateStr,s,d,A,L)}}
            @keydown=${y?this._onKeyActivate(()=>this._openCalendar(e.targetDateStr,s,d,A,L)):p}
            role=${y?"button":p}
            tabindex=${y?"0":p}
