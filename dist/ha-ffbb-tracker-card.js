...
`;var Ye=class extends T{static get properties(){return{hass:{attribute:!1},_config:{state:!0},_translations:{state:!0}}}static get styles(){return oe`
      .card-version {
        text-align: right;
        font-size: 0.75em;
        font-weight: 500;
            ...
            <div class="modal-body" tabindex="0">
              ${n.length>0?h`
                    <table class="standings-table">
                      ...
                      <tbody>
                        ${n.map(i=>{let f=i.team_name||i.name||"",g=u(f)||c(f);return h`
            ...
            <div class="modal-body" tabindex="0">
              ${o.length>0?h`
                    <div class="calendar-list">
                      ${o.map((c,i)=>{let f=c.home_team||c.equipe_domicile||"-",g=c.away_team||c.equipe_exterieur||"-",A=c.home_logo||w,L=c.away_logo||w,k=c.score||(c.home_score!==void 0?`${c.home_score} - ${c.away_score}`:""),N=this._formatDate(c.date||c.datetime),R=l(f),S=l(g),b=R||S,x=!!(c.is_played||k),y=i===u,m="";return R?m="DOM":S&&(m="EXT"),h`
                          <div
                          ...
                          >
                            <div class="calendar-col-round">
                              ...
                              ${m?h`<span class="cal-venue-pill ${m==="DOM"?"pill-dom":"pill-ext"}">${m}</span>`:""}
                            </div>
                            <div class="calendar-col-teams">
                              <div class="cal-team-line">
                                ...
                                />
                                <span class="cal-team ${R?"my-team-text":""}">${f}</span>
                              </div>
                              <div class="cal-team-line">
                                <img
                                  class="cal-mini-logo"
    ...
    `}_renderMatchHeader(e){let{competition:t,pouleName:a,roundNumber:o}=e;return h`
      ${this._config.show_header?h`
          ...
          `:""}
    `}_renderMatchArea(e){let{isLive:t,canToggleView:a,isPostMatch:o,isGameDay:n,leftName:s,rightName:d,leftLogo:l,rightLogo:u,leftUrl:c,rightUrl:i,leftEntityId:f,rightEntityId:g,gymName:A,gymCity:L,dateFormatted:k,logoSizeClass:N,showRank:R,leftRank:S,rightRank:b,isCalendarClickable:x,isLogoClickable:y,hasStandingsData:m}=e,C=this._config?.rank_badge_style==="solid"||!!this._config?.solid_rank_badges?"rank-solid":"";return h`
      <div class="match-area">
        <div class="team-logo-cell cell-left">
          <div
            class="logo-box ${N} ${y?"clickable":""}"
            @click=${()=>this._handleLogoClick(f,c,s)}
            @keydown=${y?this._onKeyActivate(()=>this._handleLogoClick(f,c,s)):p}
            role=${y?"button":p}
            tabindex=${y?"0":p}
        ...
        ${R&&(S||b)?h`
              <div class="team-rank-cell rank-left">
                ${S?h`
                      <span
                        class="rank-badge ${this._getRankClass(S)} ${C} ${m?"clickable-badge":""}"
                        @click=${()=>{m&&this._openModal("standings")}}
                        @keydown=${m?this._onKeyActivate(()=>this._openModal("standings")):p}
                        role=${m?"button":p}
                        tabindex=${m?"0":p}
                        aria-label=${m?this._t("card.view_standings","View league standings"):p}
                        title=${m?this._t("card.view_standings","View league standings"):""}
                      >${S}</span>
                    `:""}
              </div>
              <div class="team-rank-cell rank-right">
                ${b?h`
                      <span
                        class="rank-badge ${this._getRankClass(b)} ${C} ${m?"clickable-badge":""}"
                        @click=${()=>{m&&this._openModal("standings")}}
                        @keydown=${m?this._onKeyActivate(()=>this._openModal("standings")):p}
                        role=${m?"button":p}
                        tabindex=${m?"0":p}
