...
// ha-ffbb-tracker-card.js in a given module graph, and Vitest gives each
// test file its own fresh graph. A second import in the same file (or in
// components.test.js / interactions.test.js, which already import this
...
describe("window.customCards registration", () => {
  it("pushes a new entry when none exists yet for this card type", async () => {
    ...
    expect(window.customCards[0].type).toBe("ffbb-tracker-card");
  });
});
