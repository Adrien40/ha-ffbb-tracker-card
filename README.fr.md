...
<p align="center">📸 <a href="docs/SCREENSHOTS.fr.md">Plus de captures d'écran</a></p>
...
* ⏱️ **Affichage adaptatif de la rencontre :**
  ...
  * **Après-match :** score final grand format — votre score est en gras et dans la couleur d'accentuation pour une lecture immédiate, celui de l'adversaire reste plus discret — plus un badge de résultat plein à fort contraste (**Victoire**, **Défaite** ou **Nul**).
...
| `title` | `string` | `""` | Titre personnalisé, affiché en préfixe devant le titre dynamique qui suit l'état du match (« Prochain match », « Match en direct », « Dernier match »). Laissez vide pour n'afficher que ce titre dynamique. |
...
| `standings_popup_detailed` | `boolean` | `false` | Affiche le tableau détaillé complet (mêmes colonnes que la carte classement indépendante) dans la popup ouverte depuis les pastilles de classement, à la place du tableau simple (#, équipe, Pts, J G P N). |
| `display_mode` | `string` | `match` | Carte(s) à afficher : `match` (uniquement la carte de match, comme aujourd'hui), `standings` (uniquement la carte classement indépendante), ou `both` (les deux). La carte classement reprend les colonnes du site officiel FFBB : Pts, Rencontres (J G P N), I, Pén., For., Déf., Pénalités (Arb / Ent) et Points (M / E / D) — seule votre équipe est surlignée ; sur petit écran, le tableau défile horizontalement, les colonnes rang et équipe restant épinglées. S'il n'y a pas encore de classement disponible, la carte reste affichée avec un message "aucun classement disponible" au lieu de disparaître. Avec `both`, le tableau garde une police compacte alignée sur la popup, et le titre de la carte reprend cette même taille réduite ; seule (`standings`), le tableau et le titre s'affichent tous deux à la taille normale de la carte de match. Le classement simple reste dans la popup ouverte depuis les pastilles de classement, sauf si `standings_popup_detailed` est activé. |
| `standings_title` | `string` | `""` | Titre personnalisé de la carte classement indépendante (par défaut, "Classement" traduit si laissé vide). Utilisé seulement si `display_mode` vaut `standings` ou `both`. |
