...
`;var ia=["unknown","unavailable"];function vo(a,e){let t=k(a?.team_name||a?.name||""),o=k(e);return!t||!o?!1:t.includes(o)||o.includes(t)}function la(a){return a==null||a===""?"-":a}function xo(a){if(a==null||a==="")return"-";let e=Number(a);return Number.isFinite(e)?e>0?`+${e}`:String(e):a}function yo(a){let e=Number(a);return a==null||a===""||!Number.isFinite(e)||e===0?"":e>0?"diff-pos":"diff-neg"}var wo={matches:[{key:"table_played",fb:"J",full:"table_played_full",fullFb:"Games played",get:a=>a.played},{key:"table_wins",fb:"G",full:"table_wins_full",fullFb:"Wins",get:a=>a.wins??a.won},{key:"table_losses",fb:"P",full:"table_losses_full",fullFb:"Losses",get:a=>a.losses??a.lost},{key:"table_draws",fb:"N",full:"table_draws_full",fullFb:"Draws",get:a=>a.draws}],singles:[{key:"table_irregularities",fb:"I",full:"table_irregularities_full",fullFb:"Irregularities",get:a=>a.irregularities},{key:"table_penalties",fb:"P\xE9n.",full:"table_penalties_full",fullFb:"Penalties",get:a=>a.total_penalties},{key:"table_forfeits",fb:"For.",full:"table_forfeits_full",fullFb:"Forfeits",get:a=>a.forfeits},{key:"table_defaults",fb:"D\xE9f.",full:"table_defaults_full",fullFb:"Defaults",get:a=>a.defaults}],penalties:[{key:"table_referee",fb:"Arb",full:"table_referee_full",fullFb:"Referee penalties",get:a=>a.referee_penalties},{key:"table_coach",fb:"Ent",full:"table_coach_full",fullFb:"Coach penalties",get:a=>a.coach_penalties}],points:[{key:"table_scored",fb:"M",full:"table_scored_full",fullFb:"Points scored",get:a=>a.points_for},{key:"table_conceded",fb:"E",full:"table_conceded_full",fullFb:"Points conceded",get:a=>a.points_against},{key:"table_diff",fb:"D",full:"table_diff_full",fullFb:"Points difference",get:a=>a.points_diff,signed:!0}]};function ct(a){return h`
    <img
      class="col-team-logo"
      src=${a||A}
      alt=""
      @error=${e=>{e.target.src.endsWith(A)||(e.target.src=A)}}
    />
  `}function dt({rows:a,teamName:e,displayTeamName:t,t:o}){let{matches:n,singles:s,penalties:r,points:d}=wo,i=c=>o(`card.${c.key}`,c.fb),u=c=>o(`card.${c.full}`,c.fullFb),f=c=>h`<th class="sub-head" scope="col"><abbr title=${u(c)}>${i(c)}</abbr></th>`,p=(c,l)=>{let _=c.get(l);return c.signed?h`<td class="num ${yo(_)}">${xo(_)}</td>`:h`<td class="num">${la(_)}</td>`};return h`
      ...
      <table class="standings-table standings-table-detailed">
        <thead>
          <tr class="group-row">
            ...
            ${s.map(c=>h`<th class="single-head" rowspan="2" scope="col" title=${u(c)}><abbr title=${u(c)}>${i(c)}</abbr></th>`)}
            <th class="group-head" colspan=${r.length} scope="colgroup">${o("card.table_group_penalties","Penalties")}</th>
            <th class="group-head" colspan=${d.length} scope="colgroup">${o("card.table_group_points","Points")}</th>
          </tr>
          <tr class="sub-row">
            ${n.map(f)}
            ${r.map(f)}
        ...
        <tbody>
          ${a.map(c=>{let l=vo(c,e),_=l&&t?t:c.team_name||c.name||"-";return h`
              <tr class=${l?"highlight-row":""}>
                <td class="pos-cell col-pos">${c.position||c.rank||"-"}</td>
                <td class="col-team">${ct(c.logo_url)}<span>${_}</span></td>
                <td class="pts-cell">${la(c.points??c.pts)}</td>
                ${n.map(x=>p(x,c))}
                ${s.map(x=>p(x,c))}
                ${r.map(x=>p(x,c))}
                ${d.map(x=>p(x,c))}
  ...
  `}function ca({standings:a,poule:e,competition:t,teamName:o,displayTeamName:n,accentColor:s,title:r,icon:d,compact:i,t:u}){let f=De(a),p=e&&!ia.includes(e)?e:"",c=t&&!ia.includes(t)?t:"",l=r||u("card.standings_title","Standings"),_=d!==void 0?d:"mdi:format-list-numbered";return h`
    <ha-card class="standings-card ${i?"standings-compact":""}" style="--ffbb-accent-color: ${s};">
      ...
      ${c?h`<div class="standings-card-subtitle">${c}</div>`:m}
...
`;var ht=class extends P{static get properties(){return{hass:{attribute:!1},_config:{state:!0},_translations:{state:!0}}}static get styles(){return q`
    ...
    `}};customElements.get("ffbb-tracker-card-editor")||customElements.define("ffbb-tracker-card-editor",ht);var pt=class extends P{static get properties(){return{hass:{attribute:!1},_config:{state:!0},_activeModal:{state:!0},_manualView:{state:!0},_matchIndex:{state:!0}}}constructor(){super(),this._translationsLang="en",this._translations=ae("en"),this._activeModal=null,this._manualView=null,this._matchIndex=null,this._modalTrigger=null}static async getConfigElement(){return document.createElement("ffbb-tracker-card-editor")}static getStubConfig(){return{entity:"",...ye}}getCardSize(){let e=this._config?this._resolveEntities():null;return ea({config:this._config,standings:e?.rank?.attributes?.standings})}getGridOptions(){return{columns:12,min_columns:9,min_rows:3}}setConfig(e){if(!e.entity)throw new Error("Please define an entity from the FFBB Tracker integration.");this._config={...ye,...e},this._warnIfInvalidAccentColor()}_warnIfInvalidAccentColor(){let{accent_color:e,custom_accent_color:t}=this._config,o=String(t??"").trim();e!=="custom"||!o||we(o)||this._warnedAccentColor!==o&&(this._warnedAccentColor=o,console.warn(`[FFBB Tracker Card] custom_accent_color "${o}" is not a valid CSS color (use e.g. #1e88e5 or "blue"): the default orange is used.`))}willUpdate(e){if(super.willUpdate(e),e.has("hass")&&this.hass){let t=Oe(this.hass);t!==this._translationsLang&&(this._translationsLang=t,this._translations=ae(t))}}_t(e,t=""){return Ne(this._translations,e,t)}_colon(){return this._translationsLang==="fr"?"\xA0:":":"}_getRankClass(e){if(!e||this._config?.rank_badge_style==="none")return"";let o=String(e).trim().match(/^(\d+)/);if(!o)return"";let n=parseInt(o[1],10);return n===1?"rank-gold":n===2?"rank-silver":n===3?"rank-bronze":""}_resolveEntities(){return Zt(this._config.entity,this.hass?.states)}_localeInfo(){let e=this.hass?.locale?.language||this.hass?.language||"en-US";return{language:e,hour12:ta(this.hass?.locale?.time_format,e)}}_formatDate(e){let{language:t,hour12:o}=this._localeInfo();return lt(e,t,{hour12:o})}_openMaps(e,t){let o=encodeURIComponent(`${e} ${t}`.trim());window.open(`https://www.google.com/maps/search/?api=1&query=${o}`,"_blank","noreferrer")}_openCalendar(e,t,o,n,s){if(!e||e==="unknown"||e==="unavailable")return;let r=new Date(e);if(isNaN(r.getTime()))return;let d=new Date(r.getTime()+7200*1e3),i=l=>l.toISOString().replace(/[-:]|\.\d{3}/g,""),u=`${t} vs ${o}`,f=`${n} ${s}`.trim(),p=this._t("card.calendar_details","FFBB match: {home} vs {away}").replace("{home}",t).replace("{away}",o),c=`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(u)}&dates=${i(r)}/${i(d)}&details=${encodeURIComponent(p)}&location=${encodeURIComponent(f)}`;window.open(c,"_blank","noreferrer")}_handleLogoClick(e,t,o){let n=this._config.logo_click_action||"team_url";n==="team_url"?t?window.open(t,"_blank","noreferrer"):(console.warn(`[FFBB Tracker Card] No URL found for team: "${o}".`),e&&this._fireMoreInfo(e)):n==="more-info"&&e&&this._fireMoreInfo(e)}_fireMoreInfo(e){let t=new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:e}});this.dispatchEvent(t)}_openModal(e){this._modalTrigger=this.shadowRoot?.activeElement??null,this._activeModal=e}_closeModal(){this._activeModal=null}updated(e){if(super.updated(e),!e.has("_activeModal"))return;let t=e.get("_activeModal");if(this._activeModal&&!t)this.shadowRoot?.querySelector(".modal-card")?.focus();else if(!this._activeModal&&t){let o=this._modalTrigger;this._modalTrigger=null,o&&o.isConnected&&typeof o.focus=="function"&&o.focus()}}_onModalKeydown(e){if(e.key==="Escape"){e.stopPropagation(),this._closeModal();return}if(e.key!=="Tab")return;let t=e.currentTarget.querySelector(".modal-card");if(!t)return;let o=[...t.querySelectorAll('[tabindex]:not([tabindex="-1"])')];if(o.length===0){e.preventDefault(),t.focus();return}let n=o[0],s=o[o.length-1],r=this.shadowRoot?.activeElement;e.shiftKey&&(r===n||r===t)?(e.preventDefault(),s.focus()):!e.shiftKey&&r===s&&(e.preventDefault(),n.focus())}_setManualView(e){this._manualView=e,this._matchIndex=null}_handleChevronClick(e,t){if(t.hasCalendar&&t.calendarMatches.length>1){let o=e==="prev"?t.currentIndex-1:t.currentIndex+1;if(o>=0&&o<t.calendarMatches.length){this._matchIndex=o;let n=t.calendarMatches[o];this._manualView=n.is_played||n.score?"last":"next"}}else this._setManualView(e==="prev"?"last":"next")}_selectCalendarMatch(e,t){this._matchIndex=e,this._manualView=t?"last":"next",this._closeModal()}_onKeyActivate(e){return t=>{(t.key==="Enter"||t.key===" "||t.key==="Spacebar")&&(t.preventDefault(),e())}}_extractCalendarMatches(e){let t=this._config.entity?this.hass?.states[this._config.entity]:null;return it(e,t)}_renderModal(e,t,o,n){if(!this._activeModal)return h``;if(this._activeModal==="standings"){let s=e.rank?.attributes?.standings||[],r=De(s),d=e.poule?.attributes?.competition||"",i=e.poule?.state||"",u=r.map(l=>l.team_name||l.name||""),f=Ee(u,t),p=Ee(u,o),c=!!this._config?.standings_popup_detailed;return h`
        ...
        >
          ...
          >
            ...
            ${d?h`<div class="modal-subtitle">${d}</div>`:""}
            <div class="modal-body" tabindex="0">
              ${r.length===0?h`
                    <div class="modal-empty-text">
                      ${this._t("card.no_standings","No standings data available.")}
                    </div>
                  `:c?dt({rows:r,teamName:t,displayTeamName:n,t:(l,_)=>this._t(l,_)}):h`
                    <table class="standings-table">
                      ...
                      <tbody>
                        ${r.map(l=>{let _=l.team_name||l.name||"",x=f(_),S=x||p(_),D=x&&n?n:l.team_name||l.name||"-";return h`
                            <tr class=${S?"highlight-row":""}>
                              <td class="pos-cell">${l.position||l.rank||"-"}</td>
                              <td class="col-team">${ct(l.logo_url)}<span>${D}</span></td>
      ...
      `}if(this._activeModal==="form"){let s=e.form?.state,r=s&&s!=="unknown"&&s!=="unavailable",d=r?s:"",i=r&&e.form?.attributes?.current_streak||"",u=d.split("-").filter(Boolean);return h`
        <div
          class="modal-backdrop"
          @click=${this._closeModal}
          @keydown=${this._onModalKeydown}
            ...
            <div class="modal-body form-modal-content" tabindex="0">
              ${u.length>0?h`
                    <div class="form-badges-container">
                      ${u.map(f=>{let p="badge-draw",c=this._t("card.draw","Draw");return f==="V"||f==="W"?(p="badge-win",c=this._t("card.win","Win")):(f==="D"||f==="L")&&(p="badge-loss",c=this._t("card.loss","Loss")),h`
            ...
            <div class="modal-body" tabindex="0">
              ${s.length>0?h`
                    <div class="calendar-list">
                      ${s.map((l,_)=>{let x=l.home_team||"-",S=l.away_team||"-",D=l.score||(l.home_score!==void 0?`${l.home_score} - ${l.away_score}`:""),F=this._formatDate(l.date||l.datetime),M=u(x),b=u(S),C=M||b,w=!!(l.is_played||D),y=_===f,oe=Te({teamName:x,matchLogo:l.home_logo||l.home_team_logo,isMyTeam:M,myTeamLogo:p,nextOpponentState:e.nextOpponent?.state,nextOpponentLogo:e.nextOpponent?.attributes?.opponent_logo_url,lastOpponentState:e.lastOpponent?.state,lastOpponentLogo:e.lastOpponent?.attributes?.opponent_logo_url,standings:c}),E=Te({teamName:S,matchLogo:l.away_logo||l.away_team_logo,isMyTeam:b,myTeamLogo:p,nextOpponentState:e.nextOpponent?.state,nextOpponentLogo:e.nextOpponent?.attributes?.opponent_logo_url,lastOpponentState:e.lastOpponent?.state,lastOpponentLogo:e.lastOpponent?.attributes?.opponent_logo_url,standings:c}),T="";return M?T="DOM":b&&(T="EXT"),h`
                          <div
                          ...
                          >
                            ...
                            <div class="calendar-col-teams">
                              <div class="cal-team-line">
                                <img
                                  ...
                                  src=${oe}
                                  alt=""
                                  @error=${g=>{g.target.src.endsWith(A)||(g.target.src=A)}}
                                />
                            ...
                            <div class="calendar-col-meta">
                              ${D?h`
                                    <div class="cal-score">
                                      ${(()=>{let g=C?rt(D,M):null;return g?h`<span class="score-mine">${g.my}</span
      ...
      `}return h``}_computeViewModel(e){let{language:t,hour12:o}=this._localeInfo();return aa({entities:e,config:this._config,manualView:this._manualView,matchIndex:this._matchIndex,states:this.hass?.states,lang:this._translationsLang,locale:t,hour12:o,t:(n,s)=>this._t(n,s),isPreview:!!(this.preview||this.parentElement?.tagName==="HUI-CARD-PREVIEW"||this.closest&&this.closest("hui-card-preview"))})}render(){if(!this.hass||!this._config)return h``;let e=this._resolveEntities();if(!e)return h`
    ...
    `}_renderMatchHeader(e){let{competition:t,pouleName:o,roundNumber:n}=e;return h`
      ${this._config.show_header?h`
          ...
          `:""}
    `}_renderMatchArea(e){let{isLive:t,canToggleView:o,isPostMatch:n,isGameDay:s,leftName:r,rightName:d,leftLogo:i,rightLogo:u,leftUrl:f,rightUrl:p,leftEntityId:c,rightEntityId:l,gymName:_,gymCity:x,dateFormatted:S,logoSizeClass:D,showRank:F,leftRank:M,rightRank:b,isCalendarClickable:C,isLogoClickable:w,hasStandingsData:y}=e,E=this._config?.rank_badge_style==="solid"?"rank-solid":"";return h`
      <div class="match-area">
        <div class="team-logo-cell cell-left">
          <div
            class="logo-box ${D} ${w?"clickable":""}"
            @click=${()=>this._handleLogoClick(c,f,r)}
            @keydown=${w?this._onKeyActivate(()=>this._handleLogoClick(c,f,r)):m}
            role=${w?"button":m}
            tabindex=${w?"0":m}
          ...
          >
            <img
              ...
              src=${i}
              alt=""
              @error=${T=>{T.target.src.endsWith(A)||(T.target.src=A)}}
            />
          </div>
        </div>
        ...
        <div class="center-meta-wrapper">
          ...
          <div
            ...
            @click=${()=>{C&&this._openCalendar(e.targetDateStr,r,d,_,x)}}
            @keydown=${C?this._onKeyActivate(()=>this._openCalendar(e.targetDateStr,r,d,_,x)):m}
            role=${C?"button":m}
            tabindex=${C?"0":m}
            aria-label=${C?this._t("card.add_to_calendar","Add to Google Calendar"):m}
        ...
        <div class="team-logo-cell cell-right">
          <div
            class="logo-box ${D} ${w?"clickable":""}"
            @click=${()=>this._handleLogoClick(l,p,d)}
            @keydown=${w?this._onKeyActivate(()=>this._handleLogoClick(l,p,d)):m}
            role=${w?"button":m}
            tabindex=${w?"0":m}
    ...
    `}_renderFooter(e){let{gymName:t,gymCity:o,hasValidForm:n,isPreview:s,displayFormSequence:r,displayFormStreak:d,showFormBlock:i}=e;return h`
      ${i?h`
            <div
            ...
            >
              ...
              <span class="form-sequence">${r}</span>${d?h`<span class="form-streak">(${d})</span>`:""}
