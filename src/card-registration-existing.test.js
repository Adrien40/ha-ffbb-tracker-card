...
// reads) is a module-level side effect in ha-ffbb-tracker-card.js: it
...
// module registry (see the "N workers spawned, one per file" note in
// `npm test`'s coverage output), but re-importing the same specifier a
// second time within one file just returns the already-cached module --
// it does not re-run the top-level side effect a second time.
...
describe("window.customCards registration", () => {
  it("replaces the existing entry in place instead of pushing a duplicate", async () => {
    window.customCards = [
      ...
      { type: "ffbb-tracker-card", name: "Stale name from a previous load" },
    ];

    await import("./ha-ffbb-tracker-card.js");

    expect(window.customCards).toHaveLength(2);
    const entry = window.customCards.find((c) => c.type === "ffbb-tracker-card");
    expect(entry.name).not.toBe("Stale name from a previous load");
    expect(entry.name).toMatch(/^FFBB Tracker v/);
  });
});
