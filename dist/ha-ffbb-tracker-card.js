...
`;var De=class extends k{static get properties(){return{hass:{attribute:!1},_config:{state:!0},_translations:{state:!0}}}static get styles(){return G`
      .card-version {
        text-align: right;
        font-size: 0.75em;
        font-weight: 500;
            ...
            <div class="modal-body">
              ${r.length>0?d`
                    <table class="standings-table">
                      ...
                      <tbody>
                        ${r.map(i=>{let c=x(i.team_name||i.name||""),u=x(t),h=x(a),m=!!(c&&u&&(c===u||c.includes(u)||u.includes(c))),f=!!(c&&h&&(c===h||c.includes(h)||h.includes(c)));return d`
            ...
            <div class="modal-body">
              ${o.length>0?d`
                    <div class="calendar-list">
                      ${o.map(l=>{let i=l.home_team||l.equipe_domicile||"-",c=l.away_team||l.equipe_exterieur||"-",u=l.score||(l.home_score!==void 0?`${l.home_score} -${l.away_score}`:""),h=this._formatDate(l.date||l.datetime),m=x(i),f=x(c),_=x(t),E=!!(m&&_&&(m===_||m.includes(_)||_.includes(m))),L=!!(f&&_&&(f===_||f.includes(_)||_.includes(f)));return d`
    ...
    `}_renderMatchHeader(e){let{competition:t,pouleName:a,roundNumber:o}=e;return d`
      ${this._config.show_header?d`
          ...
          `:""}
    `}_renderMatchArea(e,t){let{isValidState:a,isLive:o,canToggleView:r,isPostMatch:s,isGameDay:l,leftName:i,rightName:c,leftLogo:u,rightLogo:h,leftUrl:m,rightUrl:f,leftEntityId:_,rightEntityId:E,gymName:L,gymCity:w,dateFormatted:M,logoSizeClass:y,showRank:j,leftRank:T,rightRank:A,isCalendarClickable:$,isLogoClickable:v,hasStandingsData:g}=e,U=this._config?.rank_badge_style==="solid"||!!this._config?.solid_rank_badges?"rank-solid":"";return d`
      <div class="match-area">
        ...
        <div class="center-meta-wrapper">
          ...
          <div
            ...
            @click=${()=>{$&&this._openCalendar(t.nextDate?.state,i,c,L,w)}}
            @keydown=${$?this._onKeyActivate(()=>this._openCalendar(t.nextDate?.state,i,c,L,w)):p}
            role=${$?"button":p}
            tabindex=${$?"0":p}
        ...
        ${j&&(T||A)?d`
              <div class="team-rank-cell rank-left">
                ${T?d`
                      <span
                        class="rank-badge ${this._getRankClass(T)} ${U} ${g?"clickable-badge":""}"
              ...
              <div class="team-rank-cell rank-right">
                ${A?d`
                      <span
                        class="rank-badge ${this._getRankClass(A)} ${U} ${g?"clickable-badge":""}"
                        @click=${()=>{g&&this._openModal("standings")}}
                        @keydown=${g?this._onKeyActivate(()=>this._openModal("standings")):p}
                        role=${g?"button":p}
                        tabindex=${g?"0":p}
