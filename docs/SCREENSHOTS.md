...
> ⚠️ This screenshot predates a few visual tweaks and isn't fully up to date: calendar-row logos are now bigger (22px, were 16px), team names in the standings tables no longer truncate, standings rows now show a small team logo too, and calendar rows are a bit more compact. A refreshed screenshot is welcome (see below).

From left to right:

* **Upcoming match** (`display_mode: match`, the default) -- team logos, clickable round/journée badge, recent form streak, and venue with a Google Maps link.
...
* **Standings** (`display_mode: standings`, bottom-left card) -- the full pool table, with your own team highlighted, a small logo per row, and horizontal scroll on narrow screens.
